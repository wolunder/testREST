...
public class CadController {
    ...
    public ResponseEntity<String> addFileToCadObject(@PathVariable("id") String id, @RequestParam("fileCad") MultipartFile downloadFile) {
        LOG.info("работает метод добавления файла");
    ...
    public ResponseEntity<String> setArchiveStatusToCadObject(@PathVariable("id") String id) {
        LOG.info("работает удаление для User ");
    ...
    @GetMapping("/get-farm-obj/{farm}")
    public ResponseEntity<List<CadObject>> findListObjByFarm(@PathVariable("farm") String farm) {
        List<String> farmlist = new ArrayList<>();
        farmlist.add("ОАО АФПЗ \"Победа\"");
        farmlist.add("ОАО САФ \"Русь\"");
        for (String str : farmlist) {
            str.toLowerCase();
            if (str.contains(farm.toLowerCase())) {
                LOG.info(str + "   " + farm);
                List<CadObject> listObjFarm = cadObjectService.getListFarmObj(str);
                return new ResponseEntity<>(listObjFarm, HttpStatus.OK);
